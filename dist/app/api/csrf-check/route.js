...
function _POST() {
  _POST = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee(req) {
    var _yield$req$json, code, issues, formRegex, forms, fetchRegex, xhrRegex, vulnerable;
    return _regenerator().w(function (_context) {
      while (1) switch (_context.n) {
        ...
        case 1:
          ...
          issues = []; // Check for <form> tags without CSRF token fields
          formRegex = /<form[\s\S]*?>[\s\S]*?<\/form>/gi;
          forms = code.match(formRegex) || [];
          forms.forEach(function (form, index) {
            var hasToken = /name=['"]?csrf(token)?['"]?/i.test(form);
            if (!hasToken) {
              issues.push("Form ".concat(index + 1, " does not include a CSRF token."));
            }
          });

          // Check for fetch or XMLHttpRequest POST without CSRF header
          fetchRegex = /fetch\([`'"][^'"]+[`'"],\s*{[\s\S]*?method:\s*['"]POST['"][\s\S]*?}\)/gi;
          xhrRegex = /new\s+XMLHttpRequest\(\)[\s\S]*?open\(['"]POST['"]/gi;
