...
const Navbar = () => {
  ...
  return (
    <nav
      className={`absolute top-0 left-0 w-full flex items-center justify-between md:justify-start md:gap-10 ${textColor} font-semibold font-inter text-base px-2 lg:px-8 py-1 z-50 bg-transparent`}
    >
      ...
      >
        <svg
          className={`w-6 h-6 ${textColor} transition-all duration-300 ease-in transform ${
      ...
      <ul className="hidden md:flex md:font-normal lg:font-semibold md:justify-center md:items-center md:w-full md:gap-10 w-screen z-10">
        ...
        <li className="relative group" ref={toolkitRef}>
          <button
            className={`${navLinkClasses('/toolkit')} flex items-center gap-1`}
          ...
          <ul
            className={`absolute top-full mt-2 left-0 w-40 ${
              textColor === 'text-white' ? 'bg-black/70' : 'bg-white/80'
            } backdrop-blur-md rounded-md shadow-lg py-2 z-30 transition-opacity duration-200 ${
              toolkitOpen ? 'opacity-100 visible' : 'opacity-0 invisible'
            }`}
          >
            <li>
              <Link
      ...
      <ul
        className={`z-20 absolute top-0 left-0 py-3 flex flex-col items-center gap-6 md:hidden ${
          textColor === 'text-white' ? 'bg-black/70' : 'bg-white/70'
        } backdrop-blur-xl w-full rounded-b-2xl transition-all duration-500 ease-in-out transform ${
          menuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-10 pointer-events-none'
        }`}
      >
        <li>
          <Link href="/" className={navLinkClasses('/')} onClick={() => setMenuOpen(false)}>
            Base
          </Link>
        </li>
        <li>
          <Link href="/about" className={navLinkClasses('/about')} onClick={() => setMenuOpen(false)}>
            Our Vision
          </Link>
        </li>
        ...
        <li>
          <Link href="/services" className={navLinkClasses('/services')} onClick={() => setMenuOpen(false)}>
            Services
          </Link>
        </li>
      </ul>
            ...
            {showDropdown && (
              <div
                className={`absolute right-0 mt-2 ${
                  textColor === 'text-white' ? 'bg-[#1a1a1a]' : 'bg-white border border-gray-200'
                } ${textColor} rounded-lg shadow-lg py-2 px-4 z-50`}
              >
                <button
