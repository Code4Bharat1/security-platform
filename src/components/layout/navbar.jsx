...
export default function Navbar() {
  ...
  const adminDashboardHref = useMemo(() => {
    // 1. Explicit environment variable check (sanitized and ignore localhost when in prod build)
    if (process.env.NEXT_PUBLIC_ADMIN_URL && !process.env.NEXT_PUBLIC_ADMIN_URL.includes('localhost')) {
    ...
    // 2. Dynamic browser hostname detection for live deployment fallback
    if (typeof window !== 'undefined') {
      const host = window.location.hostname;
      if (host === 'security.nexcorealliance.com' || host.endsWith('.nexcorealliance.com')) {
        return 'https://security-admin.nexcorealliance.com/admin/dashboard';
      }
