...
useEffect(() => {
    const fetchHistory = async () => {
      try {
        const res = await fetch("https://zypher-api.code4bharat.com/ssrf-checker/history");
...
const handleScan = async (e) => {
  ...
  try {
    const res = await fetch("https://zypher-api.code4bharat.com/ssrf-checker/test", {
      method: 'POST',
...
const handleDelete = async (id) => {
  try {
    const res = await fetch(`/api/ssrf-checker/delete/${id}`, {
      method: 'DELETE',
    });
