...
useEffect(() => {
    const fetchHistory = async () => {
      try {
        const res = await fetch('http://localhost:4180/api/ssrf-checker/history');
...
const handleScan = async (e) => {
  ...
  try {
    const res = await fetch('http://localhost:4180/api/ssrf-checker/test', {
      method: 'POST',
...
const handleDelete = async (id) => {
  try {
    const res = await fetch(`http://localhost:4180/api/ssrf-checker/delete/${id}`, {
      method: 'DELETE',
    });
