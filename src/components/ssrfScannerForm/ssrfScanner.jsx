...
useEffect(() => {
    const fetchHistory = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_PROD_API_URL}/ssrf-checker/history`);
...
const handleDelete = async (id) => {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_PROD_API_URL}/ssrf-checker/delete/${id}`, {
      method: 'DELETE',
    });
