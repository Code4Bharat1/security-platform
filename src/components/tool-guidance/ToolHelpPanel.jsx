...
function FieldStatusPill({ status }) {
  ...
  return (
    <span className={`rounded-full border px-2.5 py-1 text-[0.65rem] font-bold uppercase tracking-wider ${tone}`}>
      {label}
    </span>
  );
}
              ...
              <section className="space-y-3 rounded-lg border border-white/10 bg-black/25 p-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-white/40">Current input</p>
                  </div>
                  <FieldStatusPill status={activeFieldState?.status || "info"} />
                </div>
                <p className="text-sm leading-relaxed text-white/70">{activeField.description}</p>
                <InlineValidationMessage {...activeFieldState} example="" />
                {activeField.examples?.length ? (
                  <div className="rounded-lg border border-white/8 bg-black/20 px-3 py-2">
                    <p className="text-xs text-white/40">Example</p>
                    <code className="mt-1 block break-all text-xs font-mono text-white/90">
                ...
                {activeField.commonMistakes?.length ? (
                  <div>
                    <p className="mb-2 text-xs font-bold uppercase tracking-wider text-white/40">Avoid</p>
              ...
              <section className="space-y-3">
                ...
                {activeErrors.length ? (
                  <div className="rounded-2xl border border-rose-500/20 bg-rose-500/10 p-4">
                    <div className="flex items-start gap-3">
                      <ShieldX className="mt-0.5 h-4 w-4 text-rose-400" />
