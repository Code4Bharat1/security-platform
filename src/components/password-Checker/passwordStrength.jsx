    ...
    >
      {copied ? "Copied!" : "Copy"}
          ...
          <div className="mt-3">
            ...
            <div className="mt-1 text-sm text-center text-slate-300">{label}</div>
          </div>

          {/* Criteria */}
          <div className="mt-4 text-sm text-slate-300">
            <span className="mr-2">{data?.length ?? 0} characters containing:</span>
          ...
          {/* Time to crack */}
          <div className="mt-4">
            <div className="text-xs text-slate-400">Time to crack your password:</div>
            <div className="text-3xl font-semibold">
              {data?.crackTime?.human ?? "—"}
            </div>
            {data && (
              <div className="text-xs text-slate-500 mt-1">
                (Assuming {data.crackTime.assumptions.guessesPerSecond.toLocaleString()} guesses/sec)
              </div>
            )}
          </div>
          ...
          {data?.advice?.length ? (
            ...
            <div className="mt-4 text-sm text-emerald-400">Great! Your password looks strong.</div>
          ) : null}

          {loading && <div className="mt-3 text-xs text-slate-400">Analyzing…</div>}
          {err && <div className="mt-3 text-sm text-red-400">Error: {err}</div>}
        </section>

        <footer className="text-xs text-slate-500">
          Your passwords are never stored. The strength is computed on the server and nothing is logged.
        </footer>
      </div>
    </div>
  );
