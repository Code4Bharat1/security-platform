          ...
          <div className="flex flex-wrap justify-center gap-6 sm:gap-8 lg:gap-10 w-full max-w-screen-xl mx-auto items-stretch">
            ...
            {loading ? (
              ...
              <BlogCard
                image={
                  dynamicNews?.image
                    ? `/api/image-proxy?url=${encodeURIComponent(dynamicNews.image)}&t=${new Date().getTime()}`
                    : "/blogs/3.png"
                }
...
function BlogCard({ image, text, link, isStatic, title, publishedAt, error }) {
  ...
  // Check correctly for proxy path
  const hasDynamicImage = image && image.startsWith("/api/image-proxy");
