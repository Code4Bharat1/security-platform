...
useEffect( () =>{
  const fetchHistory = async () => {
   try{
   const res = await fetch('http://localhost:5000/api/broken-access-control/reports');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('http://localhost:5000/api/broken-access-control/broken-test', {
        method: 'POST',
...
const handleDelete = async (id) => {
  ...
  try {
    const res = await fetch(`http://localhost:5000/api/broken-access-control/delete/${id}`, {
      method: 'DELETE',
    });
