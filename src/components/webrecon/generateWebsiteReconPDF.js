...
const getFindingDetails = (section, key, value) => {
  ...
  // Try partial port match if key starts with Port
  if (section === "Ports") {
...
export const generateWebsiteReconPDF = async (scanResult, setPdfProgress, existingDoc = null) => {
  ...
  try {
    ...
    // Resolve detailed info for findings
    ...
    // Dynamic Overall Risk Assessment
    ...
    // Loop and render detailed findings
    mappedFindings.forEach((f) => {
      // If we don't have enough space for the detailed findings box (needs about 70mm), add a page
      if (297 - y < 70) {
        doc.addPage();
        y = 25;
      }
