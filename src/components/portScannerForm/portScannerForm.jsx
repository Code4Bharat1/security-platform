  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (parsed.mode === "single") {
        ...
        const portsObj = Object.fromEntries(portsArray.map((p) => [p.port, p]));
        ...
        setResult({
          ...
          suspicious: portsArray.filter(p => p.open && p.risk === "High").map(p => p.port),
          summary: { 
            total, 
            open: openCount, 
    ...
    <div className="min-h-screen w-full bg-black">
      ...
      <div className="max-w-4xl mx-auto px-2 py-8 ">
        <div className="bg-red-600 text-white p-4 rounded-t-lg border border-white">
          <h2 className="text-lg font-semibold text-center">Enhanced Port Scanner</h2>
        ...
        <div className="bg-gray-900 p-6 rounded-b-lg border border-white border-t-0">
          <form onSubmit={handleSubmit}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              {/* Port Filter */}
              <div>
                <label className="block text-white text-sm mb-2">Port Filter</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
              ...
              {/* Hostname Resolution */}
              <div>
                <label className="block text-white text-sm mb-2">Additional Options</label>
          ...
          {loading && (
            <div className="mt-6 flex flex-col items-center justify-center p-6 bg-gray-800 rounded border border-white">
              ...
              <p className="text-gray-400 text-sm mt-1">
                {includeHostnames ? "Resolving hostnames..." : "Analyzing port status..."}
              </p>
            </div>
          )}
            ...
            <div className="mt-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                ...
                <StatCard 
                  ...
                  value={`${result.summary?.filtered || 0} / ${result.summary?.total || 0}`} 
                  highlight 
                />
                <StatCard 
                  label="Open Ports" 
              ...
              <div className="mb-4 p-3 bg-gray-800 border border-white rounded">
                <div className="flex flex-wrap items-center gap-4 text-sm">
                  ...
                  <span className="text-white font-medium capitalize">{result.filter || filter}</span>
                  <span className="text-gray-400">•</span>
                  <span className="text-gray-400">Range:</span>
                  <span className="text-white font-medium">{result.portRange || "Custom"}</span>
                  {result.includeHostnames && (
                    <>
                      <span className="text-gray-400">•</span>
                      <div className="flex items-center gap-1">
                        <Server className="h-4 w-4 text-green-400" />
                        <span className="text-green-400 text-sm">Hostnames Resolved</span>
                      </div>
                    </>
                  )}
                </div>
              ...
              {/* Export Options */}
              <div className="mb-4">
                <ExportBar baseName={`port-scan-${result.host}-${filter}`} rows={rows} />
              </div>
                ...
                <div className="overflow-x-auto rounded border border-white">
                  <table className="min-w-full text-sm bg-gray-800">
                    <thead className="bg-red-600">
                      <tr>
                        <th className="px-3 py-2 text-left font-medium text-white border-b border-white">Port</th>
                        <th className="px-3 py-2 text-left font-medium text-white border-b border-white">Status</th>
                        <th className="px-3 py-2 text-left font-medium text-white border-b border-white">Service</th>
                        {includeHostnames && (
                          <th className="px-3 py-2 text-left font-medium text-white border-b border-white">Hostname</th>
                        )}
                        <th className="px-3 py-2 text-left font-medium text-white border-b border-white">Risk</th>
                        <th className="px-3 py-2 text-left font-medium text-white border-b border-white">Description</th>
                      </tr>
                    </thead>
                    <tbody>
                      {rows.map((r, i) => (
                        <tr key={`${r.port}-${i}`} className={i % 2 ? "bg-gray-800" : "bg-gray-900"}>
                          <td className="px-3 py-2 text-white font-mono border-b border-gray-700">{r.port}</td>
                          <td className={`px-3 py-2 border-b border-gray-700 ${getStatusStyle(r.status)}`}>
                            {r.status}
                          </td>
                          <td className="px-3 py-2 text-gray-300 border-b border-gray-700">{r.service}</td>
                          {includeHostnames && (
                            <td className="px-3 py-2 text-blue-400 border-b border-gray-700 font-mono text-xs">
                              {r.hostname}
                            </td>
                          )}
                          <td className={`px-3 py-2 border-b border-gray-700 ${getRiskStyle(r.risk)}`}>
                            {r.risk}
                          </td>
              ...
              {rows.length === 0 && (
                <div className="p-6 bg-gray-800 border border-white rounded text-center">
                  <p className="text-gray-400">No ports found matching the current filter criteria.</p>
                  <p className="text-gray-500 text-sm mt-1">
                    Try changing the filter to "All Ports" to see complete results.
                  </p>
                </div>
              )}
            </div>
