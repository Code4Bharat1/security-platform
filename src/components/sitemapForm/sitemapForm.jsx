...
export default function SitemapForm() {
  ...
  const downloadPDF = async () => {
    ...
    doc.text(
      `Crawl Depth: ${sitemapData.summary?.crawlDepth ?? depth}`,
      40,
      72
    );
            ...
            <div className="border rounded-lg p-4 bg-gray-200">
              ...
              {(sitemapData.summary?.broken ?? 0) > 0 && (
                <div className="mb-3 p-3 rounded border border-yellow-300 bg-yellow-50 text-yellow-800">
                  ⚠ Some URLs in sitemap are <b>4xx</b>. Please review broken
                  links below.
                </div>
              )}
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-4">
                <Stat
                  label="Total Pages"
                  value={
                    sitemapData.summary?.totalPages ??
                    sitemapData.pagesFound ??
                    0
                  }
                />
                <Stat
