...
function OurApproch({ data }) {
    return (
        <div className="min-h-screen">
            <h2 className=" tex-center text-2xl sm:text-4xl md:text-6xl font-inter font-bold underline underline-offset-8 md:underline-offset-12 decoration-[#9d7af0]/70 shadow-lg text-white mt-10 mb-20 mx-5 text-center">
                Our Approch
            </h2>
            <div className="grid grid-cols-6 gap-x-4 gap-y-8 text-white text-center">
                ...
                {data.firstRow.map((item, index) => (
                    <div key={index} className="col-span-2 flex justify-center items-center">
                        <div className="rounded-xl font-semibold text-sm md:text-xl border-dashed border-2 border-[#A580FF]/50 p-4 w-full max-w-sm aspect-video bg-white/10 backdrop-blur-xl h-full flex items-center justify-center">
                            {item}
                        </div>
                    </div>
                ))}
